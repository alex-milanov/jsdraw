...
gui.View2D = function(_conf, _scene, _editor){
			...
			switch(_view.editor.params["obj-mode"]){
				...
				case "scale":
					...
					if( _view.scene.selected.geometry.type === "BoxGeometry" &&
						_view.scene.selected.geometry.scale(_view.mod, new ext.Vector2(
...
gui.View2D.prototype.drawCube = function(_obj){
...
gui.View2D.prototype.drawBlock = function(_obj){
...
gui.View2D.prototype.drawBox = function(_obj, _lineDash, _strokeStyle, _showCoords){
		...
		//if(index == 4){
			...
			if(objSpan2.x !== 0 && objSpan2.y !== 0){
				...
				if(that.editor.indexes.length === 0 || that.editor.indexes.indexOf(index+"")>-1){
						that.ctx.fillText("a("+(new ext.Vector3()).copy(_quad.a).toVector2(_mod).toArray().join(", ")+"), "+objStart2.toArray().join(", "),objStart2.x,objStart2.y);
						that.ctx.fillText("b("+(new ext.Vector3()).copy(_quad.b).toVector2(_mod).toArray().join(", ")+"), "+objStart2.y,(objStart2.x+objSpan2.x),objStart2.y);
						that.ctx.fillText("c("+(new ext.Vector3()).copy(_quad.c).toVector2(_mod).toArray().join(", ")+"), "+objStart2.clone().add(objSpan2).toArray().join(", "),objStart2.x+objSpan2.x,objStart2.y+objSpan2.y);
						that.ctx.fillText("d("+(new ext.Vector3()).copy(_quad.d).toVector2(_mod).toArray().join(", ")+"), "+objStart2.x+", "+(objStart2.y+objSpan2.y),objStart2.x,objStart2.y+objSpan2.y);
					}
					_indexes.push(index);
...
gui.View2D.prototype.refresh = function(_entities){
