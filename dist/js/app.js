		...
		value: function init() {
			...
			[].slice.call(dom.querySelectorAll('[class*=\'-toggle\']')).map(function (el) {
				return _rxLite.Observable.fromEvent(el, 'click').map(function () {
					...
					var toggleParam = el.getAttribute('data-toggle-param');
					var toggleSelf = el.getAttribute('data-toggle-self');
					console.log(toggleRefEl, toggleClass, toggleParam, toggleSelf);
					toggleRefEl.classList.toggle(toggleClass);
					if (toggleSelf) {
						console.log(toggleSelf);
						toggleSelf.split(' ').map(function (cls) {
...
},{"rx-lite":2}],5:[function(require,module,exports){
'use strict';
...
var View = function (_gui$Element) {
	_inherits(View, _gui$Element);
	...
	_createClass(View, [{
		...
		value: function init() {
			...
			var fromDomEvent = function fromDomEvent(dom, type) {
				return _rxLite.Observable.fromEvent(dom, type).map(mapEventType(type));
			};

			var mouseDown$ = fromDomEvent(this.dom, 'mousedown');
			var mouseMove$ = fromDomEvent(this.dom, 'mousemove');
			var mouseUp$ = fromDomEvent(this.dom, 'mouseup');
			...
			var mouseEvent$ = _rxLite.Observable.merge(mouseDown$, mouseMove$, mouseUp$, mouseLeave$).map(function (_ref) {
				...
				switch (type) {
					case 'mousedown':
						currentPath = new _path2.default({
							lineJoin: 'round',
							brush: Object.assign({}, _this2.context.brush),
							color: _this2.context.colors.fg || '#000000'
						});
      ...
      ctx.shadowColor = 'rclientWidthgb(0, 0, 0)';*/
		...
		value: function refresh() {
			...
			this.layers.forEach(function (layer) {
				layer.canvas.width = _this3.dom.clientWidth;
				layer.canvas.height = _this3.dom.clientHeight;
	...
	return View;
}(_gui2.default.Element);

exports.default = View;

},{"../gfx/path":3,"../gui":5,"rx-lite":2}]},{},[6]);
