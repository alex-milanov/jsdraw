...
var switchFn = function switchFn(value, options) {
  return options[value] || options['default'] || false;
};
...
var change = function change(prop, value) {
  return stream.onNext(function (state) {
    return Object.assign({}, state, switchFn(prop, {
      'tool': { tool: value },
...
var addPath = function addPath(index, path) {
  return stream.onNext(function (state) {
    return Object.assign({}, state, {
      layers: [].concat(state.layers.slice(0, index), [Object.assign({}, state.layers[index], { paths: [].concat(state.layers[index].paths, [path]) })], state.layers.slice(index + 1))
    });
  });
};
...
var Path = function () {
  function Path(_ref) {
    var color = _ref.color,
        ...
        lineJoin = _ref$lineJoin === undefined ? 'round' : _ref$lineJoin,
        _ref$lineCap = _ref.lineCap,
        lineCap = _ref$lineCap === undefined ? 'round' : _ref$lineCap,
        _ref$shadowBlur = _ref.shadowBlur,
        shadowBlur = _ref$shadowBlur === undefined ? 5 : _ref$shadowBlur;
...
var canvasService = require('./services/canvas');
...
var mapEventType = function mapEventType(type) {
	return function (ev) {
		return { ev: ev, type: type };
	};
};
var fromDomEvent = function fromDomEvent(dom, type) {
	return $.fromEvent(dom, type).map(mapEventType(type));
};

var mouseDown$ = fromDomEvent(document, 'mousedown');
var mouseMove$ = fromDomEvent(document, 'mousemove');
var mouseUp$ = fromDomEvent(document, 'mouseup');
var mouseLeave$ = fromDomEvent(document, 'mouseleave');

var drawEvent$ = $.merge(mouseDown$, mouseMove$, mouseUp$, mouseLeave$).filter(function (_ref) {
	var ev = _ref.ev,
	    type = _ref.type;
	return ev.target === document.querySelector('.viewport');
...
},{"./actions":12,"./services/canvas":15,"./ui":16,"./util/vdom":17,"rx":2}],15:[function(require,module,exports){
'use strict';

var Rx = require('rx');
var $ = Rx.Observable;
var Path = require('../gfx/path');
...
var init = function init(_ref) {
  ...
  var mouseEvent$ = $.merge(mouseDown$, mouseMove$, mouseUp$, mouseLeave$).withLatestFrom(state$, function (_ref2, state) {
    var type = _ref2.type,
        ev = _ref2.ev;

    console.log(ev);
    if (ev.target !== document.querySelector(dom)) return;
    dom = ev.target;
    console.log(type, ev);
    switch (type) {
      case 'mousedown':
...
var refresh = function refresh(_ref3) {
  var dom = _ref3.dom,
      state = _ref3.state,
...
},{"../gfx/path":13,"rx":2}],16:[function(require,module,exports){
'use strict';
...
module.exports = function (_ref) {
  var state = _ref.state,
      actions = _ref.actions;
  return div('.gui', [div('.header', [div('.toolbox', tools.map(function (tool) {
    return button(tool.cls, {
      class: {
        selected: tool.name === state.tool
      },
      on: {
        click: function click(ev) {
          return actions.change('tool', tool.name);
        }
      }
    });
  })), h1('.logo', [i('.fa.fa-paint-brush'), span('JSDraw')]), div('.presets', [input('#brush-size', {
    props: { type: 'range', step: 1, min: 0, max: 100, value: state.brush.size },
    on: { change: function change(ev) {
        return actions.change('brush-size', ev.target.value);
      } }
  }), span('#brush-size-value', state.brush.size), div('.color-preset', [input('#color-fg', {
    props: { type: 'color', value: state.color.fg },
    on: { change: function change(ev) {
        return actions.change('color-fg', ev.target.value);
      } }
  }), input('#color-bg', {
    props: { type: 'color', value: state.color.bg },
    on: { change: function change(ev) {
        return actions.change('color-bg', ev.target.value);
      } }
  })])])]), div('.viewport', [div('.view', [div('.layers', state.layers.map(function (layer, index) {
    return canvas('#layer-' + index, { style: { zIndex: 100 + index } });
  })), canvas('.interaction')])])]);
};

},{"../util/vdom":17}],17:[function(require,module,exports){
'use strict';
...
},{"snabbdom":10,"snabbdom/h":3,"snabbdom/modules/class":6,"snabbdom/modules/eventlisteners":7,"snabbdom/modules/props":8,"snabbdom/modules/style":9}]},{},[14]);
